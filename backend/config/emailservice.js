...
const sendEmail = async ({ to, subject, htmlContent }) => {
    try {
        ...
        if (!BREVO_API_KEY || !SENDER_EMAIL) {
            throw new Error("Email configuration missing (BREVO API KEY or EMAIL KEY");
        }
        ...
        const response = await fetch("https://api.brevo.com/v3/smtp/email", {
            ...
            body: JSON.stringify({
                sender: { name: "CarsWorld", email: SENDER_EMAIL },
                to,
                subject,
                htmlContent,
            })
...
// to sendverification email
export const sendVerificationEmail = async (email, name, otp) => {
    return sendEmail({
        to: [{ email, name }],
        subject: "Your Verification Code - CarsWorld",
...
//to sendForgotPasswordEmail
export const sendForgotPasswordEmail = async (email, name, otp) => {
    return sendEmail({
        to: [{ email, name }],
        subject: "Reset Your Password - CarsWorld",
